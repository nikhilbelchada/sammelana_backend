defmodule SammelanaWeb.Auth.KeyStore do
  @moduledoc """
  The KeyStore handles fetching public keys from Google's servers to verify public keys with

  ## Warnings

  By default SammelanaWeb.Auth.KeyStore will stop when the initial fetch failed. This behavior can be
  ...
  def start_link(_) do
    GenServer.start_link(__MODULE__, %{}, name: SammelanaWeb.Auth.KeyStore)
  end
  ...
  def init(_) do
    ...
    case SammelanaWeb.Auth.KeySource.fetch_certificates() do
      :error ->
          ...
          :stop ->
            {:stop,
             """
               ...
               If you want to run offline during tests or development, add the following key to your config
  ...
  def find_or_create_ets_table do
    case :ets.whereis(SammelanaWeb.Auth.KeyStore) do
      :undefined -> :ets.new(SammelanaWeb.Auth.KeyStore, [:set, :public, :named_table])
      table -> table
    end
  end
  ...
  defp store_data_to_ets(data) do
    ...
    |> Enum.each(fn {key, value} ->
      :ets.insert(SammelanaWeb.Auth.KeyStore, {key, value})
    end)
  end
